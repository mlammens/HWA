...
```{r}
# Tree richness
plot_tree_richness <- trees %>%
  group_by(Land_Manager, Plot) %>%
  summarize(Tree_Richness = n_distinct(Species_Name, na.rm = TRUE),
            n_Trees = n(), .groups = "drop")

site_tree_richness <- trees %>%
  group_by(DEP_DEC_Site) %>%
  summarize(Tree_Richness = n_distinct(Species_Name, na.rm = TRUE),
            n_Trees = n(), .groups = "drop")

# Plant richness
plot_plant_richness <- ground_cover %>%
  group_by(Land_Manager, Plot) %>%
  summarize(Plant_Richness = n_distinct(Species, na.rm = TRUE),
            .groups = "drop")

site_plant_richness <- ground_cover %>%
  group_by(DEP_DEC_Site) %>%
  summarize(Plant_Richness = n_distinct(Species, na.rm = TRUE),
            .groups = "drop")

## Merge richness with HWA severity data
plot_richness_and_severity <- plot_sev %>%
  select(Land_Manager, Plot, Stand_Health) %>%
  distinct() %>%
  left_join(plot_tree_richness, by = c("Land_Manager","Plot")) %>%
  left_join(plot_plant_richness, by = c("Land_Manager","Plot"))

site_richness_and_severity <- site_sev %>%
  select(DEP_DEC_Site, Stand_Health) %>%
  distinct() %>%
  left_join(site_tree_richness,  by = "DEP_DEC_Site") %>%
  left_join(site_plant_richness, by = "DEP_DEC_Site")

# Reorder Stand_Health levels
plot_richness_and_severity <- plot_richness_and_severity %>% # removed richness_combined
  mutate(Stand_Health = factor(Stand_Health, 
                               levels = c("Healthy", "Moderate", "Severe"), 
                               ordered = TRUE))

site_richness_and_severity <- site_richness_and_severity %>%
  mutate(Stand_Health = factor(Stand_Health, 
                               levels = c("Healthy", "Moderate", "Severe"), 
                               ordered = TRUE))
```
...
## Plots and stats for richness


```{r}
# Plot-level
plant_richness_plot <- ggplot(plot_richness_and_severity, 
                              aes(x = Stand_Health, y = Plant_Richness)) +
  geom_boxplot() +
  labs(title = "Plot-level", 
       x = "Stand Health", y = "Plant Species Richness") +
  coord_cartesian(ylim = c(0, 44)) +
  theme_minimal()
#plant_richness_plot

tree_richness_plot <- ggplot(plot_richness_and_severity, 
                             aes(x = Stand_Health, y = Tree_Richness)) +
  geom_boxplot() +
  labs(title = "Plot-level", 
       x = "Stand Health", y = "Tree Species Richness") +
  coord_cartesian(ylim = c(0, 14)) +
  theme_minimal()
#tree_richness_plot

## Site-level
plant_richness_site <- ggplot(site_richness_and_severity,
                              aes(x = Stand_Health, y = Plant_Richness)) +
  geom_boxplot() +
  labs(
    title = "Site-level",
...
tree_richness_site <- ggplot(site_richness_and_severity,
                             aes(x = Stand_Health, y = Tree_Richness)) +
  geom_boxplot() +
  labs(
    title = "Site-level",
...
aligned <- align_plots(plant_richness_plot, tree_richness_plot, 
                       plant_richness_site, tree_richness_site,
                       align = "hv", axis = "tblr")
plot_grid(aligned[[1]], aligned[[3]], 
          aligned[[2]], aligned[[4]],
